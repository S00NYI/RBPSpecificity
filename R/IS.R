# Functions related to calculating Specificity

#' Calculate Specificity
#'
#' @description Calculates the specificity for a given motif based
...
#'   and 'Score'.
#' @param motif Character string, the specific motif for which to calculate specificity.
...
#'   If "specific", returns a single specificity value for the target motif. If "all",
#'   returns a data frame with specificity values for every motif.
#'
#' @return A single numeric specificity value if `return_type = "specific"`, or a data
#'   frame with 'MOTIF' and 'Specificity' columns if `return_type = "all"`.
#' @export
#'
#' @examples
...
returnSpecificity <- function(motif_enrichment, motif = NULL, return_type = "specific") {
    # --- Input Validation ---
    ...
    if (return_type == "specific") {
        ...
        spec_value <- calSpecificity(scores = all_scores, target_score = target_score)
        return(spec_value)
    } else { # return_type == "all"
        # --- Calculate for all motifs (vectorized) ---
        message("Calculating specificity for all motifs...")
        all_scores <- motif_enrichment$Score
        median_score <- stats::median(all_scores, na.rm = TRUE)

        if (is.na(median_score) || median_score == 0) {
            spec_values <- rep(NA_real_, nrow(motif_enrichment))
        } else {
            spec_values <- all_scores / median_score
        }
...
#' Calculate Specificity Core Logic
#'
...
#' @keywords internal
calSpecificity <- function(scores, target_score) {
