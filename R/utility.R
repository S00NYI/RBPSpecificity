...
scrambleDNA <- function(seq) {
    # Robust check for NULL, NA, or empty string content using anyNA
    if (is.null(seq) || anyNA(seq) || nchar(as.character(seq)) == 0) {
...
#' @importFrom methods is
#' @keywords internal
validateGetSequenceInputs <- function(granges_obj, genome_obj, extension, min_length) {
...
extendCoordinatesStrandAware <- function(granges_obj, extension) {
    ext_5prime <- as.integer(extension[1])
    ext_3prime <- as.integer(extension[2])

    strand_info <- as.character(GenomicRanges::strand(granges_obj))
    plus_strand_idx <- strand_info %in% c("+", "*")
    minus_strand_idx <- strand_info == "-"

    new_start <- GenomicRanges::start(granges_obj)
    new_end <- GenomicRanges::end(granges_obj)
    ...
    return(list(start = new_start, end = new_end))
}

filterSeqlevelsAndTrim <- function(current_gr, genome_obj, min_length, verbose = TRUE) {
    ...
    if (length(valid_seqlevels) < length(GenomeInfoDb::seqlevelsInUse(current_gr))) {
        ...
        if (num_removed_sl > 0 && verbose) {
            message(num_removed_sl, " ranges removed due to seqlevels not present in the genome object.")
        }
    }

    if (length(current_gr) == 0) {
        if (verbose) message("No ranges remaining after filtering for seqlevels present in genome object.")
        return(NULL)
    }
    ...
    if (length(trimmed_gr) == 0) {
        if (verbose) message("No valid ranges remaining after trimming to chromosome boundaries.")
        return(NULL)
    }
...
reattachMetadata <- function(trimmed_gr, original_mcols, original_names) {
    ...
    if (!is.null(final_original_indices)) {
        ...
        trimmed_gr$..original_index.. <- NULL
...
getSequence <- function(granges_obj, genome_obj, extension = c(0, 0), min_length = 1, verbose = TRUE) {
    ...
    current_gr$..original_index.. <- seq_along(current_gr)

    coords <- extendCoordinatesStrandAware(current_gr, extension)
    final_widths <- coords$end - coords$start + 1

    valid_idx <- final_widths >= min_length
    num_removed_short <- sum(!valid_idx)
    if (num_removed_short > 0 && verbose) {
        message(num_removed_short, " ranges removed due to final length < ", min_length, " after extension/trimming.")
    }

    if (!any(valid_idx)) {
        if (verbose) message("No ranges remaining after filtering for minimum length.")
        return(Biostrings::DNAStringSet())
    }

    current_gr <- current_gr[valid_idx]
    IRanges::ranges(current_gr) <- IRanges::IRanges(start = coords$start[valid_idx], end = coords$end[valid_idx])

    trimmed_gr <- filterSeqlevelsAndTrim(current_gr, genome_obj, min_length, verbose)
    if (is.null(trimmed_gr)) {
        return(Biostrings::DNAStringSet())
    }

    trimmed_gr <- reattachMetadata(trimmed_gr, original_mcols, original_names)
...
generateBkgSet <- function(peak_gr, genome_obj, K,
    ...
    # Shift original peaks cleanly using IRanges::shift
    shifted_gr <- peak_gr
    IRanges::ranges(shifted_gr) <- IRanges::shift(IRanges::ranges(peak_gr), shift = rand_distances)
    ...
    # Pass verbose = FALSE to getSequence to avoid redundant output
...
generateBkgSetBatched <- function(peak_gr, genome_obj, min_seq_length,
    ...
    # 2. Generate all random distances and shift at once using IRanges::shift
    rand_distances <- genRNDist(N = total_regions, X = bkg_min_dist, max_dist = bkg_max_dist)
    shifted_gr <- batched_gr
    IRanges::ranges(shifted_gr) <- IRanges::shift(IRanges::ranges(batched_gr), shift = rand_distances)
