...
## Calculating Variation Sensitivity (VS)

VS quantifies how sensitive an RBP is to single nucleotide changes in its 
preferred motif.

```{r calc-ms}
# Get VS as a matrix (shows sensitivity at each position)
vs_matrix <- returnVS(hnrnpc_rbns, output_type = "matrix")
print(vs_matrix)

# Get VS as a single number (average sensitivity)
vs_hnrnpc <- returnVS(hnrnpc_rbns, output_type = "number")
vs_eif4g2 <- returnVS(eif4g2_rbns, output_type = "number")

message("HNRNPC VS: ", round(vs_hnrnpc, 4))
message("EIF4G2 VS: ", round(vs_eif4g2, 4))
```
...
position. Here we compare VS profiles for both RBPs:

```{r plot-vs-hnrnpc, fig.cap="Variation sensitivity profile for HNRNPC (high specificity RBP)."}
plotVS(hnrnpc_rbns)
```

```{r plot-vs-eif4g2, fig.cap="Variation sensitivity profile for EIF4G2 (low specificity RBP)."}
plotVS(eif4g2_rbns)
```
...
# Calculate IS and VS from enrichment data
is_eclip_hnrnpc <- returnIS(hnrnpc_enrichment)
vs_eclip_hnrnpc <- returnVS(hnrnpc_enrichment, output_type = "number")

is_eclip_eif4g2 <- returnIS(eif4g2_enrichment)
vs_eclip_eif4g2 <- returnVS(eif4g2_enrichment, output_type = "number")

message("HNRNPC eCLIP - IS: ", round(is_eclip_hnrnpc, 2), ", VS: ", round(vs_eclip_hnrnpc, 4))
message("EIF4G2 eCLIP - IS: ", round(is_eclip_eif4g2, 2), ", VS: ", round(vs_eclip_eif4g2, 4))
```
...
# VS Profile plots
plotVS(hnrnpc_enrichment)
plotVS(eif4g2_enrichment)
```
...
for (i in seq_along(rbps)) {
  ...
  results$VS[i] <- returnVS(temp_df, output_type = "number")
}
